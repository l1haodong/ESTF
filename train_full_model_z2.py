...
from configs.config_z2 import learning_rate, num_epochs, train_batch_size, val_batch_size\
    , model_save_path, save_freq, device_id, mode, ValData, delta
from util.utils_z2 import to_psnr, print_log, validation, adjust_learning_rate, findLastCheckpoint
from model.GDConvNet_z2 import L1_Charbonnier_loss, Net
    ...
    for epoch in range(initial_epoch, num_epochs):
        ...
        for batch_id, train_data in enumerate(train_data_loader):
            ...
            if not (batch_id % 100):
                ...
                writer.add_scalar('Loss/train_z2', loss.data.item(), timestep)
                #print(oup_loss.data.item())
                writer.add_scalar('Loss_oup/train_z2', oup_loss.data.item(), timestep)
                #print(mid_oup_loss.data.item())
                writer.add_scalar('Loss_mid_oup/train_z2', mid_oup_loss.data.item(), timestep)
                writer.add_scalar('Loss_g_Spatial/train_z2', torch.mean(g_Spatial).data.item(), timestep)
                #writer.add_scalar('PSNR/train', psnrs.avg, timestep)
                #writer.add_scalar('SSIM/train', ssims.avg, timestep)
                writer.add_scalar('Lr/train_z2', optimizer.param_groups[-1]['lr'], timestep)
