...
class DeformConv3d(nn.Module):
    ...
    def forward(self, img1, img2, img4, img5, offset_source):
        ...
        weight1 = self.get_weight(h_z, 0, img1_feature.size(1))
        weight2 = self.get_weight(h_z, 1/3, img1_feature.size(1))
        weight4 = self.get_weight(h_z, 2/3, img1_feature.size(1))
        weight5 = self.get_weight(h_z, 1, img1_feature.size(1))

        central_offset = (img1_feature * weight1 + img2_feature * weight2 + img4_feature * weight4 + img5_feature * weight5)/\
                         (weight1 + weight2 + weight4 + weight5)
    ...
    def get_weight(self, h_z, z_pos, C):
        d_z = (h_z - z_pos) ** 2

        weight = 1/(d_z + 1e-15)
        #　weight = torch.exp(-10*(self.scale*(d_x + d_y) + d_z))
