...
from model.GDConvNet_3D_z2_grid_skip import L1_Charbonnier_loss,CharbonnierFunc
...
class DeformConv3d(nn.Module):
    def __init__(self, inc, outc, offset_source_channel, kernel_size=3, padding=1, stride=1, bias=None, modulation=False):
        ...
        def Subnet_offset(ks):
            return torch.nn.Sequential(
                ...
                torch.nn.Conv2d(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1),
                torch.nn.ReLU(inplace=False),
                torch.nn.Conv2d(in_channels=64, out_channels=ks * 2, kernel_size=3, stride=1, padding=1),
                torch.nn.ReLU(inplace=False),
                # torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True),
                # torch.nn.Conv2d(in_channels=ks * 2, out_channels=ks * 2, kernel_size=3, stride=1, padding=1)
            )
        ...
        self.moduleAlpha4 = Subnet_offset(self.kernel_size ** 2)
        #self.moduleBeta4 = Subnet_offset(self.kernel_size ** 2)
        self.moduleAlpha5 = Subnet_offset(self.kernel_size ** 2)
        #self.moduleBeta5 = Subnet_offset(self.kernel_size ** 2)

        # image：（64->5*5）（5，2，1）

        self.z_conv1 = Subnet_weight(self.kernel_size ** 2)
        self.z_conv2 = Subnet_weight(self.kernel_size ** 2)
        self.z_conv4 = Subnet_weight(self.kernel_size ** 2)
        self.z_conv5 = Subnet_weight(self.kernel_size ** 2)
        ...
        if modulation:
            ...
            self.m_conv = Subnet_m(self.kernel_size ** 2)
    ...
    def forward(self, img1, img2, img4, img5, offset_source):
        ...
        offset_1 = self.moduleAlpha1(offset_source)
        offset_2 = self.moduleAlpha2(offset_source)
        offset_4 = self.moduleAlpha4(offset_source)
        offset_5 = self.moduleAlpha5(offset_source)
        ...
        z1 = self.z_conv1(offset_source)
        z2 = self.z_conv2(offset_source)
        z4 = self.z_conv4(offset_source)
        z5 = self.z_conv5(offset_source)
        softmax = torch.nn.Softmax(dim=1)
        z = softmax(torch.cat((z1.unsqueeze(1),z2.unsqueeze(1),z4.unsqueeze(1),z5.unsqueeze(1)),dim =1))
        z1 = z[:, 0, :, :, :].squeeze(1)
        z2 = z[:, 1, :, :, :].squeeze(1)
        z4 = z[:, 2, :, :, :].squeeze(1)
        z5 = z[:, 3, :, :, :].squeeze(1)
        ...
        # get_z_weight 得到了权重，不仅是4张帧（4个支持点对应4次函数）的权重，也是25个点的权重，但是到这一步还是加权之后的采样点的形式
        central_offset = img1_feature * self.get_z_weight1(z1, "1", img1_feature.size(1)) + \
                         img2_feature * self.get_z_weight1(z2, "2", img1_feature.size(1)) + \
                         img4_feature * self.get_z_weight1(z4, "4", img1_feature.size(1)) + \
                         img5_feature * self.get_z_weight1(z5, "5", img1_feature.size(1))
        ...
        if self.modulation:
            ...
            #print(m.shape)
            #print(central_offset.shape)
            central_offset *= m

        # central_offset：   B, 3, 256, 256, 25
    ...
    def get_z_weight1(self, z, mode, C):
        weight = z.contiguous().permute(0, 2, 3, 1)             # B, H, W, N
