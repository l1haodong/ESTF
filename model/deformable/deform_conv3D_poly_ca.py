...
class DeformConv3d(nn.Module):
    ...
    def get_2D_output(self, x, offset):
        ...
        # (b, h, w, 2N)
        # torch.view等方法操作需要连续的Tensor，所以用contiguous()？
        ...
        q_lt = p.detach().floor()
        q_rb = q_lt + 1
        # torch.clamp：压缩 input 到 min-max
        # 压缩了H 压缩了W，然后再最后一个维度上cat
    ...
    def _get_p_0(self, h, w, N, device, dataType):
        # 数据的矢量化，适用于生成网格型数据，接受两个一维数组生成两个二维矩阵
